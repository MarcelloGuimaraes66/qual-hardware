...
Os botões PDF, XLSX e JSON geram um relatório combinado com as três políticas: mínimo técnico, recomendado e N+1. Cada opção única contém primeiro a referência comercial interna e, em seguida, requisitos mínimos funcionais sem marca. O PDF começa com um resumo didático; o XLSX acrescenta abas de referência comercial, especificação neutra, matriz de comprovação e competitividade. O JSON usa `capacity-recommendation-export/5.0.0`.
...
O SQLite v8 preserva o catálogo legado e registra, em histórico append-only, cada campo oficial por componente, o valor original e normalizado, unidade, fonte, localização da evidência, hash, conflito e completude. Dados antigos sem evidência de campo continuam visíveis, mas são marcados como legados/ambíguos e não liberam contratação. Somente `validated_local` e `extrapolated_high` aparecem como aptos para aquisição; as demais opções ficam separadas como planejamento ou referência.
