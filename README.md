...
Os botões PDF, XLSX e JSON geram relatórios com as três políticas: mínimo técnico, recomendado e N+1. O PDF baixado como **qual-hardware-recomendacoes.pdf** mantém a estrutura do relatório comparativo original: narrativa, três configurações, outras máquinas, carga e três propostas completas. Somente depois dessa Parte I começa a Parte II, numerada por máquina, componente, especificação e grupo técnico, com os campos oficiais disponíveis. Os parágrafos explicativos são justificados e usam português do Brasil. A auditoria extensa permanece no XLSX/JSON; os requisitos sem marca ficam no anexo neutro separado. O XLSX inclui a aba **Especificações detalhadas** e o JSON usa `capacity-recommendation-export/6.0.0` com `componentTechnicalSpecifications`.
