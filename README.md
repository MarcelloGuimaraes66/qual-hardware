...
Os pacotes desktop contêm Electron, fontes, dependências dos relatórios, helper de topologia e benchmark nativo. Não existe arquivo auxiliar obrigatório para abrir o modo diagnóstico; FFmpeg/FFprobe, `llama-server`, Qwen3-VL GGUF e `mmproj` continuam externos e são necessários para medições reais. No Windows, a certificação da recepção RTSP também usa o [Simulador de RTSP](https://github.com/edguimkit/simulador-rtsp). Antes da tela principal, **Verificação do ambiente** identifica esses componentes, testa streams locais e localiza todos os pares Qwen3-VL + `mmproj`. A aplicação calcula um orçamento conservador com RAM, VRAM, memória unificada e CPU, testa automaticamente os pares recomendados para Core/Core Max e permite escolher manualmente somente pares já aprovados no ensaio local. O mesmo GGUF pode ser usado em Windows, macOS e Ubuntu; o backend do llama.cpp muda conforme o equipamento. O usuário final não precisa instalar Node.js. Componentes ausentes recebem orientação e link oficial; nenhum download ou instalador é executado automaticamente. Os pacotes desktop internos não são assinados e podem exibir SmartScreen ou Gatekeeper; a publicação de cada GitHub Release é manual.
...
O SQLite v13 preserva integralmente v1–v12 e registra ambiente, componentes, autotestes e avisos, além das tentativas de capacidade, ensaios Qwen3-VL, perfis de memória e relatórios vinculados por SHA-256. A topologia multi-dispositivo, a evidência por CPU/GPU, as fronteiras de capacidade e os planos de frota continuam preservados. Antes da primeira migração persistente, o aplicativo cria uma cópia SQLite consistente no subdiretório `schema-backups`. Dados sem evidência suficiente continuam visíveis como diagnóstico e não liberam contratação.

Modelos Qwen3-VL não são liberados pelo nome ou pelo tamanho do GGUF. O Qual Hardware escolhe um `llama-server` que exponha a GPU detectada, calcula os hashes do modelo, do `mmproj` e do runtime, aguarda `/health` e exige três respostas visuais sequenciais e duas simultâneas. O ensaio vale por 30 dias somente para o mesmo hardware, driver, backend e arquivos. Hash oficial aprovado pode participar da qualificação para aquisição; revisão desconhecida aprovada localmente permanece limitada a planejamento. Sem telemetria de VRAM suficiente, o dimensionamento mantém a reserva conservadora histórica.
...
A área permanente **Calibração de capacidade** é o único executor de calibração suportado. Ela oferece diagnóstico de 10 minutos, validação de engenharia de 60 minutos e qualificação comercial em três blocos de oito horas, com dois intervalos de 30 minutos. O próprio aplicativo cria a sessão, inicia o worker isolado, conduz FFmpeg, telemetria e Qwen local quando disponíveis, mostra o progresso, persiste o resultado atômico e encerra todos os processos e temporários pertencentes à sessão. Quando o simulador está ativo, cada câmera materializada abre uma sessão RTSP/TCP autenticada real; sem ele, o gerador interno continua apenas como diagnóstico e não libera compra.
...
O Qual Hardware não anexa ao Perceptrum em execução, não acessa banco de produção, não usa protocolo `perceptrum://` nem callback externo e bloqueia OpenAI e comunicação externa durante a calibração. O ensaio opcional do simulador fica restrito a `127.0.0.1`, path `Streaming/Channels/101`, portas 554/5541+ e credenciais de teste `admin/admin`; a URL autenticada existe somente na memória do processo e nunca é persistida no resultado. O simulador MediaMTX pode escutar em outras interfaces, portanto deve ser usado apenas em rede de teste protegida por firewall. O relatório mede sessões, frames, payload e delta de RAM no loopback, mas declara expressamente que placa, cabo e switch físicos não foram medidos. Novos resultados usam `qual-hardware-local-calibration/8.0.0`, ambiente `qual-hardware-execution-environment/3.0.0`, kernel `5.0.0` e intercâmbio `.qhcal`/`.qhcalset` v4; leitores anteriores permanecem para histórico diagnóstico.
