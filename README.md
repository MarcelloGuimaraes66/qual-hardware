...
O Qual Hardware não anexa ao Perceptrum em execução, não usa banco, credenciais, mídia real, protocolo `perceptrum://`, porta fixa ou callback externo e bloqueia OpenAI e comunicação externa durante a calibração. Se uma instalação compatível estiver presente, somente o worker `--hardware-benchmark-worker` é iniciado em workspace temporário; versões antigas continuam no modo genérico. Novos resultados usam `qual-hardware-local-calibration/7.0.0`, ambiente `qual-hardware-execution-environment/2.0.0`, kernel `4.0.0` e intercâmbio `.qhcal`/`.qhcalset` v4; leitores anteriores permanecem para histórico diagnóstico.
