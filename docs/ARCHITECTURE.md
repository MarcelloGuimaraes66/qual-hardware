...
9. At startup and every 24 hours, all three desktop packages inspect that same public channel with ETag, verify every bundle checksum/signature/sequence link, then activate hardware, components, benchmarks, prices and sources atomically. SQLite v13 preserves all v1-v12 data and adds fail-closed Qwen3-VL stack probes and resource profiles to environment/component/self-test evidence, diagnostic reports, multi-device topology, boundaries and fleet plans. A consistent SQLite backup is created before a persistent older database is migrated.
