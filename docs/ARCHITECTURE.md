...
9. At startup and every 24 hours, all three desktop packages inspect that same public channel with ETag, verify every bundle checksum/signature/sequence link, then activate hardware, components, benchmarks, prices and sources atomically. SQLite v10 preserves all v1-v9 data and adds multi-device hardware topology, per-device calibration evidence, measured capacity boundaries and fleet plans. A consistent SQLite backup is created before a persistent older database is migrated.
