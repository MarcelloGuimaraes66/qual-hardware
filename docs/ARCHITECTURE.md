...
| Perceptrum calibration control plane | packaged and smoke-tested; diagnostic-only | contract regression tests only | contract regression tests only |
...
6. Qual Hardware creates an expiring authenticated session and opens `perceptrum://calibration/run`. The protocol URI carries only the handoff version, loopback Qual Hardware origin, session UUID and a one-time nonce; the bearer token never appears in the URI. Perceptrum claims that session over loopback, downloads one exact plan/control payload and runs an isolated child under benchmark restrictions. The current Windows child is diagnostic-only and cannot generate importable evidence; the intended synthetic RTSP plus real AiQ/Qwen/Intelligence production pipeline remains gated on pinned tools and physical homologation. Manual plan/result files remain recovery paths, subject to the same importability rules.
