...
Pull requests use Node.js `24.18.0`, npm `11.16.0` and Go `1.26.5` for the same typecheck, complete test suite, native benchmark build, unpacked package and packaged smoke on `windows-2025`, `macos-26` and `ubuntu-24.04`. Linux executes Electron under Xvfb. The smoke runner validates native architecture, mandatory environment verification, desktop-only ASAR, random loopback origin, health, CSP, sandbox, the automatic catalog channel, 22-profile fallback catalog, Windows/Ubuntu/macOS recommendation targets, SQLite v13, reports, single-instance behavior, persistence and worker cleanup. Platforms without a physical Qwen3-VL stack must remain diagnostic and fail closed. On Windows it also launches the real portable bootstrap.
...
- Workload v4 distinguishes continuous VÍDEO FULL from periodic FRAME inference while keeping RTSP/base decode, Jobs, Steps, Agents, Intelligence, database/dashboard concurrency and storage policy explicit. Legacy workload versions remain readable without silently changing prior sizing or approving purchase.
...
- SQLite v13 migration must preserve every v1–v12 row and create a consistent pre-migration backup before opening a persistent older database.
- The Qwen fake server must cover success, timeout, crash, invalid payload, wrong visual answer, second concurrent request failure, cancellation and process cleanup on Windows, macOS and Ubuntu.
- Physical Windows validation must show the GPU backend selected instead of CPU, three successful sequential visual challenges, two successful concurrent requests and conservative resource fallback when per-process VRAM is unavailable.
- A physical packaged diagnostic may be exercised with `QUAL_HARDWARE_SMOKE_CERTIFY_QWEN=1`, `QUAL_HARDWARE_SMOKE_CAMERA_COUNT=<count>` and `QUAL_HARDWARE_CALIBRATION_TIME_SCALE=1`; this is a diagnostic acceptance path only and cannot promote the result to purchase qualification.
