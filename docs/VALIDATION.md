...
Pull requests use Node.js `24.18.0`, npm `11.16.0` and Go `1.26.5` for the same typecheck, complete test suite, native benchmark build, unpacked package and packaged smoke on `windows-2025`, `macos-26` and `ubuntu-24.04`. Linux executes Electron under Xvfb. The smoke runner validates native architecture, mandatory environment verification, desktop-only ASAR, random loopback origin, health, CSP, sandbox, the automatic catalog channel, 22-profile fallback catalog, Windows/Ubuntu/macOS recommendation targets, SQLite v12, reports, single-instance behavior, persistence and worker cleanup. On Windows it also launches the real portable bootstrap.
...
7. Legacy local results v1–v5 remain readable as historical diagnostic evidence. Only `qual-hardware-local-calibration/6.0.0`, plan `4.0.0`, kernel `4.0.0`, workload `4.0.0` and `exact_perceptrum` environment evidence can be purchase-eligible; new `.qhcal` and `.qhcalset` exports use version `4.0.0`.
...
- SQLite v12 migration must preserve every v1–v11 row and create a consistent pre-migration backup before opening a persistent older database.
