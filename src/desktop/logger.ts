...
export function redactDesktopLog(value: string): string {
  return value
    ...
    .replace(/("(?:token|nonce|api[_-]?key|private[_-]?key|secret)"\s*:\s*")[^"]+/gi, '$1[redacted]')
...
function rotate(baseFile: string): void {
  ...
  if (statSync(baseFile).size < MAX_LOG_BYTES) return;
...
export function installDesktopLogger(userDataPath: string): string {
  ...
  const sink = (level: "INFO" | "WARN" | "ERROR", args: unknown[]): void => {
    rotate(logFile);
    appendFileSync(logFile, `${new Date().toISOString()} ${level} ${serialize(args)}\n`, "utf8");
  };
  const original = {
