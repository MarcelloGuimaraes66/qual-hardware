...
function calibrationSessionMessage(session: CalibrationSession, fallback: string, lang: Language): string {
  if (session.error === "calibration_insufficient_temporary_space_with_reserve") {
    const free = byteLabel(session.progress?.diskFreeBytes ?? 0);
    const reserve = byteLabel(session.progress?.diskReserveBytes ?? 0);
    return lang === "pt"
      ? `Teste interrompido com segurança: há ${free} livres, abaixo da reserva obrigatória de ${reserve}. Libere espaço antes de retomar.`
      : `Test stopped safely: ${free} is free, below the mandatory ${reserve} reserve. Free disk space before resuming.`;
  }
  return session.progress?.message ?? fallback;
}
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      void api<CalibrationSession>(`/api/calibration-sessions/${session.id}`).then((next) => {
        ...
        if (next.state === "failed" || next.state === "interrupted" || next.state === "expired") {
          setDetail(calibrationSessionMessage(next, next.error ?? next.state, lang));
        }
